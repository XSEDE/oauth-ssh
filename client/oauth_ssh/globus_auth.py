...
def register_client():
    path = "/v2/api/clients"
    name = "Oauth SSH Client [" + getpass.getuser() + "@" + socket.gethostname() + "]"

    body = {"client": {"template_id": TEMPLATE_ID, "name": name}}
...
def _authenticated_request(method, path, **kw):
    ...
    return _perform_request(method, path, auth=(creds["client"], creds["secret"]), **kw)
...
def do_auth_code_grant(fqdn, force_login=False, identity=None):
    ...
    if identity is not None:
        params[
            "session_message"
        ] = "The SSH service requires that you authenticate using this identity:"
