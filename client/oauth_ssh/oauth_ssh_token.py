...
class NeedToAuthorize(OAuthSSHError):
    def __init__(self, fqdn, msg):
        super(NeedToAuthorize, self).__init__(
            msg + " Use `oauth-ssh-token authorize " + fqdn + "`."
        )
...
def token_authorize(fqdn, port, identity, client_id):
    ...
    if policy.get("permitted_idps"):
        if len(policy.get("permitted_idps", 0)) > 1 and identity is None:
            click.echo(
                "Use --identity to specify an account from one of "
    ...
    token = Auth.do_auth_code_grant(
        client_id if client_id else fqdn, force_login, identity
    )
...
#
###################################
@click.command("accounts", short_help="Display the accounts available for FQDN.")
@click.option(
    "-p",
    "--port",
    nargs=1,
